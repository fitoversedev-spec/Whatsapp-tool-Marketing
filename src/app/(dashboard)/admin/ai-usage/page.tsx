// Admin-only AI usage analytics. Reads the ai_usage log (one row per successful
// AI call — template draft or analytics report) written by the AI guardrails,
// and shows total volume, a per-person ranking (most → least), and a rough
// spend estimate. Read-only; the exact billed amount lives in the Anthropic
// Console (there's no per-key balance API), so cost here is clearly labelled an
...
// Rough estimate at Sonnet 5 intro pricing ($2 / $10 per 1M in/out tokens),
// converted at ~₹86/$. Deliberately approximate — a running sense, not a bill.
...
export default async function AiUsagePage() {
  ...
  if (!isAdmin(user.role)) redirect("/inbox");
