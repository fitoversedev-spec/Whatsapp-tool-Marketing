...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  return NextResponse.json({
    courtImage: {
      ...
      image2dUrl: row.image2dUrl,
      image3dUrl: row.image3dUrl,
      video3dUrl: row.video3dUrl,
      caption: row.caption,
      status: row.status,
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const updated = await prisma.courtImage.update({
    ...
    data: {
      ...
      ...(parsed.data.image2dUrl !== undefined && { image2dUrl: parsed.data.image2dUrl }),
      ...(parsed.data.image3dUrl !== undefined && { image3dUrl: parsed.data.image3dUrl }),
      ...(parsed.data.video3dUrl !== undefined && { video3dUrl: parsed.data.video3dUrl }),
