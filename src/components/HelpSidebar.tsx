...
export default function HelpSidebar({ user }: Props) {
  ...
  useEffect(() => {
    const stored = localStorage.getItem("ccd_help_sidebar_collapsed");
  ...
  function toggleCollapsed() {
    setCollapsed((prev) => {
      const next = !prev;
      try {
        localStorage.setItem("ccd_help_sidebar_collapsed", String(next));
      } catch {}
      return next;
    });
  }
  ...
  const navItems = [
    { id: null, label: "All Guides", icon: "\u{1F4DA}" },
  ];
    ...
    <>
      ...
      <header className="font-sans md:hidden sticky top-0 z-30 bg-white border-b border-slate-200 px-4 py-3 flex items-center gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="font-heading font-bold uppercase tracking-tight text-slate-900 truncate">Help Center</div>
          <div className="mt-0.5">
            <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-sm text-xs font-heading font-bold uppercase tracking-wide whitespace-nowrap bg-indigo-500/10 text-indigo-700">
              <span className="w-1.5 h-1.5 rounded-full bg-indigo-500" />
              Guide
            </span>
          </div>
        </div>
        <div className="w-8 h-8 rounded bg-indigo-600 text-white flex items-center justify-center font-heading font-bold text-sm">
          ?
        </div>
      </header>
      ...
      >
        ...
        <div className="px-3 pt-3">
          ...
          <div className={`hidden md:flex md:justify-center ${collapsed ? "" : "lg:hidden"}`}>
            <span className="w-2.5 h-2.5 rounded-full bg-indigo-500" title="Help Center" aria-label="Help Center" />
          </div>
        </div>
        ...
        <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
          ...
          <button
            type="button"
            onClick={() => window.open("/inbox", "fitoverse-marketing")}
        ...
        <div className={`border-t border-slate-200 p-3 md:p-2 ${collapsed ? "" : "lg:p-3"}`}>
          <div
            ...
            className={`block rounded-lg transition px-3 py-2 md:px-1 md:py-2 ${collapsed ? "" : "lg:px-3 lg:py-2"}`}
          >
