...
function styleFor(e: TimelineEntry): { border: string; badge: string; label: string } {
  if (e.kind === "reminder") {
    return e.completed
      ? { border: "border-slate-200", badge: "bg-slate-100 text-slate-600", label: "DONE" }
      : { border: "border-amber-300", badge: "bg-amber-100 text-amber-700", label: "REMINDER" };
  }
  ...
  return { border: "border-wa-green/40", badge: "bg-wa-green/10 text-wa-dark", label: "ACTIVITY" };
}
