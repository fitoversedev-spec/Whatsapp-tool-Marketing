...
export async function runMetaAdsReport(opts: {
  ...
  const tools = buildMetaAdsToolbox(opts.scope, "last_30_days");
  ...
  const system =
    ...
    "For a question about ONE specific named lead (a person by name, phone, or email), call find_lead and pass the name/number as given even if it looks misspelled — matching is fuzzy, so do not retry different spellings. " +
    ...
    "Spend and cost-per-lead (CPL) are in INR. When there IS data, always state how many campaigns or leads the numbers are based on and flag thin data (a handful of leads is not reliable). " +
